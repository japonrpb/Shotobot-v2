...
const comandos = `
        ...
        if (texto.startsWith('#instagram')) {
            ...
            try {
                ...
                const response = await axios.get('https://api.delirius.store/instagram?url=' + encodeURIComponent(link));
                if (response.data && response.data.url) {
                    await sock.sendMessage(remitente, { video: { url: response.data.url }, caption: '📥 Video de Instagram' });
                    await sock.sendMessage(remitente, { react: { text: '✅', key: mensaje.key } });
                } else {
                    throw new Error();
                }
        ...
        if (texto.startsWith('#facebook')) {
            ...
            try {
                const axios = require('axios');
                const response = await axios.get('https://api.delirius.store/facebook?url=' + encodeURIComponent(link));
                if (response.data && response.data.url) {
                    await sock.sendMessage(remitente, { video: { url: response.data.url }, caption: '📥 Video de Facebook' });
                    await sock.sendMessage(remitente, { react: { text: '✅', key: mensaje.key } });
                } else {
                    throw new Error();
                }
        ...
        if (texto.startsWith('#twitter')) {
            ...
            try {
                ...
                if (response.data && response.data.url) {
                    await sock.sendMessage(remitente, { video: { url: response.data.url }, caption: '📥 Video de Twitter/X' });
                    await sock.sendMessage(remitente, { react: { text: '✅', key: mensaje.key } });
                } else {
                    throw new Error();
                }
        ...
        // #tiktok - Video de TikTok (ya funciona)
`;

if (bot.includes('// #menu')) {
    bot = bot.replace('// #menu', comandos + '\n        // #menu');
    fs.writeFileSync('bot.js', bot);
    console.log('✅ Comandos agregados con yt-dlp y api.delirius.store');
} else {

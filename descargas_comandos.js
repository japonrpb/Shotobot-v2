...
const descargasComandos = `
        ...
        if (texto.startsWith('#tiktok')) {
            ...
            try {
                ...
                if (response.data && response.data.data) {
                    ...
                    const title = response.data.data.title || 'Video de TikTok';
                    await sock.sendMessage(remitente, { video: { url: videoUrl }, caption: '🎬 ' + title });
                    await sock.sendMessage(remitente, { react: { text: '✅', key: mensaje.key } });
                } else {
        ...
        // #youtube - Descargar video de YouTube
        if (texto.startsWith('#youtube')) {
            ...
            try {
                ...
                await sock.sendMessage(remitente, { image: { url: thumbnail }, caption: '🎬 *' + title + '*\\n📥 Descargando video...' });
                await sock.sendMessage(remitente, { video: videoStream, caption: '✅ Descargado: ' + title });
                await sock.sendMessage(remitente, { react: { text: '✅', key: mensaje.key } });
            } catch (error) {
        ...
        // #instagram - Descargar video de Instagram
        if (texto.startsWith('#instagram')) {
            ...
            try {
                ...
                const response = await axios.get('https://instagram-downloader-download-instagram-videos-stories.p.rapidapi.com/index', {
                    params: { url: link },
                    headers: { 'X-RapidAPI-Key': 'demo', 'X-RapidAPI-Host': 'instagram-downloader-download-instagram-videos-stories.p.rapidapi.com' }
                });
...
if (bot.includes('// #menu')) {
    bot = bot.replace('// #menu', descargasComandos + '\n        // #menu');

...
const comandosFinal = `
        ...
        if (texto.startsWith('#playaudio')) {
            ...
            try {
                ...
                const response = await axios.get('https://api.siputzx.my.id/api/ytplay?query=' + encodeURIComponent(busqueda));
                if (response.data && response.data.data && response.data.data.audio) {
                    ...
                    throw new Error('No se encontro audio');
                }
            } catch (error) {
        ...
        if (texto.startsWith('#youtube')) {
            ...
            try {
                const axios = require('axios');
                const response = await axios.get('https://api.siputzx.my.id/api/ytdl?url=' + encodeURIComponent(link));
        ...
        if (texto.startsWith('#instagram')) {
            ...
            try {
                const axios = require('axios');
                const response = await axios.get('https://api.siputzx.my.id/api/igdl?url=' + encodeURIComponent(link));
        ...
        if (texto.startsWith('#facebook')) {
            ...
            try {
                const axios = require('axios');
                const response = await axios.get('https://api.siputzx.my.id/api/fbdl?url=' + encodeURIComponent(link));
                if (response.data && response.data.data && response.data.data.video) {
        ...
        if (texto.startsWith('#twitter')) {
            ...
            try {
                const axios = require('axios');
                const response = await axios.get('https://api.siputzx.my.id/api/twitter?url=' + encodeURIComponent(link));
                if (response.data && response.data.data && response.data.data.video) {
...
if (bot.includes('// #menu')) {
    bot = bot.replace('// #menu', comandosFinal + '\n        // #menu');
    fs.writeFileSync('bot.js', bot);
    console.log('✅ Comandos de descarga actualizados con API funcional');
} else {
